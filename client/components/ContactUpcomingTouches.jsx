...
const ContactUpcomingTouches = props => {
  ...
  let touchId, touchObj, touchDate, touchName; //variables used to 
  ...
  for (let i = 0; i < numOfTouches; i += 1) {
    ...
    touchName = touchObj.eventName; //would like to change the variavble "name" to label

    upcomingTouchList.push(
      <li key={i} id={`cc-ut-${touchId}`}> {/* li #id = contactCard-upcomingTouch-id*/}
        <p><span className="upcoming-touch-date">{touchDate}: </span><span className="upcoming-touch-label">{touchName}</span></p>
      </li>
    )
  }
